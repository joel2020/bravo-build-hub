...
| Licensed and insured; License #8822 | `src/lib/site.ts`, `src/pages/CompanyFacts.tsx`, and visible trust copy. | Evidence-required: no issuing-authority record or current insurance evidence was supplied for this baseline. | Do not add or expand license, insurance, permit-authority, or legal-scope wording until the business provides dated source documentation; then use the exact jurisdiction and credential wording. |
| 5.0 Google rating based on 7 reviews | `src/lib/site.ts` and `src/pages/CompanyFacts.tsx` (dated there as May 2026). | Evidence-required: Maps/Business Profile data could not be reliably verified here. | Do not change, repeat in schema, or use the count as comparative proof without a dated profile capture or authorized export; use a date-qualified value only. |
| 30+ years of combined experience | `src/lib/site.ts` labels this owner-stated as of 2026-07-14. | Evidence-required: owner statement is present, but no independent proof package was supplied. | Keep only if the business approves the owner-stated wording; do not turn it into company age, technician tenure, or a comparative claim. |
| 24/7 emergency dispatch and open 24 hours | `src/lib/site.ts`, contact page, company-facts page, and emergency pages. | Operational assertion visible on production, but no dispatch logs or coverage/SLA evidence were available. | Improve: say customers should call for the next available response window; do not promise arrival windows, immediate dispatch, or same-visit repair without current operational evidence. |
| Most emergency calls fixed same visit; 60–120 minute arrival | `src/pages/EmergencyHVAC.tsx` currently makes both claims. | Evidence-required: no job or dispatch evidence was supplied. | Remove or qualify before publishing. Preserve only safety-first triage guidance and availability language that operations can support. |
| Free written estimates, written/fixed pricing, financing | `src/components/PageHero.tsx`, `src/lib/site.ts`, and service content. | Evidence-required: no dated terms, exclusions, lender terms, or pricing policy was supplied. | Do not publish a universal pricing, free-estimate, or financing promise without approved terms; clearly state any scope and exclusions once documented. |
...
| --- | --- | --- | --- | --- |
| G.C. Reliable Service | `AC repair Westchester County` | A New Rochelle service page promotes 24/7 emergency AC/HVAC, service specials, financing offers, and maintenance membership. | Publish a clearly scoped repair-to-replacement path and documented estimate/maintenance terms; do not imitate offer values without approved terms. | [G.C. Reliable AC repair](https://gcreliable.com/) |
