...
**Production state:** Corrected deployment `dpl_4wuEL7hxR71zTMHS4QzVmGGxzNxF` is live and passed the final production gate. Commit `4929ef5` is pushed to `origin/codex/seo-aeo-phase1`, and its protected completion preview passed the full preview gate. That preview has not been promoted. No merge, Search Console indexing request, Business Profile edit, or public review response has been made.
...
- **Completion promotion:** protected preview `dpl_GmhGn1qXwM72ocBoX8ZsRxFDgnxz` passed the full gate but is not live. Promoting it remains a separate external action; current production `dpl_4wuEL7hxR71zTMHS4QzVmGGxzNxF` remains unchanged and is the rollback target for that promotion.
