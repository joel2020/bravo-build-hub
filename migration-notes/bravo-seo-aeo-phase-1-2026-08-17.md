...
An active Codex heartbeat, `bravo-weekly-seo-and-aeo-kpi-check`, runs Mondays at 9:00 AM America/New_York on the project task. It compares the latest 28 settled Search Console days with the preceding period, monitors the homepage plus the four commercial priorities and Yonkers, checks sitemap/indexing and live canonical health, and incorporates Bravo GA4 results when available. It then creates one evidence-backed, non-cannibalizing local SEO blog package in `migration-notes/content-drafts/`, including SEO/AEO metadata, sources, claim review, internal links, image/alt-text guidance, and adapted Facebook and Instagram captions. The workflow is draft-first: it cannot submit indexing requests, change account settings, modify the public application, publish, deploy, push, or post to social media without fresh action-time approval. GSC Wizard tracker `ae64751b-cf51-4a9a-b474-f48b7492d416` independently monitors the five priority URLs.
