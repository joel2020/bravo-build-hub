...
| GA4 sessions | 188 / 28 settled days (Jul 22–Aug 18) | Authenticated Bravo GA4 browser report | Compare rolling 28 settled days and exclude internal/admin traffic where possible |
| Organic Search sessions | 72 / 28 settled days (38.3%) | GA4 Traffic acquisition | Track landing-page engagement and conversion events separately |
| AI Assistant sessions | 3 / 28 settled days (1.6%) | GA4 Traffic acquisition | Monitor citation/referral growth without treating it as a lead |
| Calls | No GA4 key events reported | Browser access available; phone-provider reconciliation still needed | `call_click` by page path and CTA location; reconcile with call logs |
| Forms | No GA4 key events reported; 7 `form_start` events appeared in the overview | GA4 events are collecting, but lead key-event reporting is not configured | `lead_submit` where `form=contact_lead_form` |
| Online bookings | No GA4 key events reported | GA4/CRM reconciliation needed | `lead_submit` where `form=online_booking` |
...
An active Codex heartbeat, `bravo-weekly-seo-and-aeo-kpi-check`, runs Mondays at 9:00 AM America/New_York on the project task. It compares the latest 28 settled Search Console days with the preceding period, monitors the homepage plus the four commercial priorities and Yonkers, checks sitemap/indexing and live canonical health, and incorporates Bravo GA4 results when available. It then researches, validates, and publishes one non-cannibalizing local SEO blog article using explicit senior SEO/AEO and HVAC subject-matter standards. Each article must answer a real homeowner or property-manager need, distinguish safe homeowner checks from professional HVAC work, avoid unsafe technical instructions, and substantiate material claims with current authoritative sources. Publication is allowed only after the scoped tests, production build, content/claim review, local crawl, protected-preview validation, and production verification pass; a release-critical live failure triggers rollback to the prior deployment. The workflow also saves the evidence package and adapted Facebook and Instagram captions in `migration-notes/content-drafts/`, but social posting remains draft-only pending separate authorization. It cannot change Google Business Profile or GA4 settings, post review responses, request indexing, submit/remove sitemaps, merge branches, or alter unrelated application behavior. GSC Wizard tracker `ae64751b-cf51-4a9a-b474-f48b7492d416` independently monitors the five priority URLs.
...
- Obtain Bravo Business Profile owner/manager access. Correct the GBP website/appointment URLs and confirm hours, categories, services, and service areas.
...
- Review the active weekly Search Console/site-health and content-draft heartbeat; connect durable GA4 API/OAuth, call-log, CRM, review, and revenue sources as access becomes available.
...
- On August 19, 2026, the authenticated GA4 browser reports showed 188 sessions for the settled July 22–August 18 period: 100 Direct, 72 Organic Search, 11 Referral, 3 AI Assistant, and 2 Unassigned. Organic Search produced 45 engaged sessions and a 62.5% engagement rate. The overview showed 374 page views, 16 `request_service_click` events, and 7 `form_start` events. The GA4 Lead acquisition report nevertheless showed zero new, qualified, or converted leads and zero key events, so conversion outcomes cannot yet be claimed from GA4. The GSC Wizard GA4 connector could not be used because its trial/subscription had ended; no subscription or billing change was made.
...
- A production-dependency audit of the Bravo package then identified advisories in React Router and the Supabase WebSocket chain. The completion tree upgrades `react-router-dom` to `^7.18.2`, `@supabase/supabase-js` to `^2.112.3`, Vite to `^7.3.6`, Vitest to `^4.1.10`, and PostCSS to `^8.5.23`; Vitest is now correctly classified as a development dependency. A pinned-pnpm production audit reports zero high/critical advisories attributable to the Bravo package. Five high advisories remain in the separate `artifacts/api-server` package and are outside this phase's application scope. These Bravo dependency changes are live in completion deployment `dpl_Ao4XVeLPsTcfT9JxBj4X2yRUhutx`.
- The second-pass business-claim audit found remaining first-person promises in the public financing and maintenance pages, city/service templates, and legacy articles. The completion tree replaces those promises with project-specific, evidence-gated wording and adds rendered-page plus built-bundle regression coverage. These content-safety changes are live in completion deployment `dpl_Ao4XVeLPsTcfT9JxBj4X2yRUhutx` and passed the production claim gate.
- With explicit approval, exact completion commit `4929ef593aba900ef9510a92d3cd39b3b5cf2f12` was pushed August 18, 2026 to `origin/codex/seo-aeo-phase1`. Git integration deployed that commit to protected Vercel preview `dpl_GmhGn1qXwM72ocBoX8ZsRxFDgnxz` at `https://bravo-build-9zf5kpce7-joel-carias-projects.vercel.app`. Vercel reported a ready Vite preview; the build itself completed in 14 seconds and generated 140 route pages.
- The completion preview passed the authenticated gate: 140/140 sitemap URLs returned 200 with one H1, title, description, indexable HTML robots directive, and the expected `www` canonical; all 140 retained Vercel's preview-level noindex header; 8/8 permanent redirects, 3/3 security headers, 3/3 private-route noindex checks, immutable asset caching, one-day sitemap caching, 6/6 analytics privacy assertions, and 4/4 reviewed claim groups passed. The deployment error-log query returned no error records. Production was not changed by this approval.
- With explicit approval, Vercel promoted the completion preview on August 18, 2026 at 6:38:16 PM EDT. The CLI rebuilt the exact pushed source rather than only repointing aliases; the production build log confirms branch `codex/seo-aeo-phase1` at commit `4929ef5`. Ready production deployment `dpl_Ao4XVeLPsTcfT9JxBj4X2yRUhutx` now serves `www`, apex, `app`, and the project production aliases. The previous production deployment remained live throughout the build.
- The immediate live `app`-root check returned HTTP 200 with `X-Robots-Tag: noindex, nofollow`, so rollback was not triggered. The full live gate passed: 140/140 sitemap URLs, 8/8 permanent path redirects, the apex-to-`www` redirect, 3/3 security headers, 5/5 private/app noindex checks, immutable asset caching, one-day sitemap caching, 6/6 analytics privacy assertions, and 4/4 reviewed claim groups. Vercel's deployment error-log query returned no error records.
...
- **GA4/CrUX/CWV:** browser-authenticated Bravo GA4 access is available and reporting was verified through August 18. The GSC Wizard GA4 connector is blocked by an ended trial/subscription, no durable Analytics OAuth scope is connected, and no CrUX property was available. GA4 is collecting traffic and interaction events, but its lead report shows zero key events; configuring key-event classification requires separate approval.
...
- August 18 post-launch recheck — a fresh production crawl again passed 140/140 sitemap URLs. Direct header checks confirmed the three global security headers on `www`, `X-Robots-Tag: noindex, nofollow` on both the bare `app` host and `/admin/crm`, and the permanent emergency-page redirect to its canonical service URL.
- Completion protected-preview audit (`dpl_GmhGn1qXwM72ocBoX8ZsRxFDgnxz`, commit `4929ef5`) — 140/140 sitemap URLs, preview-wide noindex isolation on 140/140 responses, 8/8 permanent redirects, 3/3 global security headers, 3/3 private-route noindex headers, asset/sitemap cache policies, 6/6 analytics privacy assertions, and 4/4 reviewed claim groups passed; Vercel returned no error logs.
- Completion production audit (`dpl_Ao4XVeLPsTcfT9JxBj4X2yRUhutx`, commit `4929ef5`) — 140/140 sitemap URLs, 8/8 permanent path redirects, apex redirect, 3/3 global security headers, 5/5 private/app noindex checks, asset/sitemap cache policies, 6/6 analytics privacy assertions, and 4/4 reviewed claim groups passed; Vercel returned no error logs. The local package-runner wrapper's TTY safeguard aborted before its first request, so the same dependency-free SEO audit script was executed directly and passed 140/140.
- The previously reported redundant `||` warning in `src/components/crm/CRMMyJobs.tsx` is removed without changing the unnamed-customer fallback; the final local production build emits no application warnings.
